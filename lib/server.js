...
function createServer(options, callback) {
    ...
    try {
        fs.unlinkSync('/var/tmp/buckets-mdplacement-ready');
    } catch (e) {
    ...
    data_placement.createDataDirector({
        ...
        var kangOpts = {
            service_name: 'buckets-mdplacement',
        ...
        socket.on('listening', function () {
            log.info('buckets-mdplacement listening on %d', options.port);
            callback(null, {
        ...
        log.info('buckets-mdplacement server setup complete, writing ready ' +
            'cookie');
        try {
            fs.writeFileSync('/var/tmp/buckets-mdplacement-ready', null);
        } catch (e) {
